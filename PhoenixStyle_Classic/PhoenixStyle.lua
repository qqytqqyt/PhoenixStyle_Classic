...
function PhoenixStyle_OnLoad()
    ...
    bigmenuchatlisten = {
        "raid", "raid_warning", "officer", "party", "guild", "say", "yell",
        "sebe"
    }
...
function PSF_OnUpdate()
        ...
        if (timertopull > 0 or pspullactiv == 1) then
            ...
            if (CurrentTimepull >= DelayTimepull) then
                ...
                if timertopull == 0 then
                    ...
                    SendChatMessage(psattackin .. " " .. timertopull .. " " ..
                                        pssec, "raid_warning")
                end
    ...
    if psmsgwaiting42 > 0 and CurrentTimepull > psmsgwaiting42 + 1.7 then
        ...
        for i, cc in ipairs(bigmenuchatlisten) do
            if string.lower(cc) == string.lower(psmsgmychat42) then
                bililine = 1
            end
        end
        if pscanannouncetable42[1] == psnamemsgsend then
            ...
            if bililine == 1 then
                ...
                if nrchatmy > 0 then
                    for i = 1, #psannouncewait42 do
                        SendChatMessage(psannouncewait42[i], "CHANNEL", nil,
                                        nrchatmy)
                        if psanounsesrazuchek == 1 then
                            psanonceinst = 1
                        end
    ...
    if psmsgwaiting43 > 0 and CurrentTimepull > psmsgwaiting43 + 1.7 then
        ...
        local bililine = 0
        for i, cc in ipairs(bigmenuchatlisten) do
            if string.lower(cc) == string.lower(psmsgmychat43) then
                bililine = 1
            end
        end
        if pscanannouncetable43[1] == psnamemsgsend then
            ...
            if bililine == 1 then
                ...
                if nrchatmy > 0 then
                    for i = 1, #psannouncewait43 do
                        SendChatMessage(psannouncewait43[i], "CHANNEL", nil,
                                        nrchatmy)
                        if psanounsesrazuchek == 1 then
                            psanonceinst = 1
                        end
    ...
    if psmsgwaiting > 0 and CurrentTimepull > psmsgwaiting + 1.7 then
        ...
        local bililine = 0
        for i, cc in ipairs(bigmenuchatlisten) do
            if string.lower(cc) == string.lower(psmsgmychat) then
                bililine = 1
            end
        end
        if pscanannouncetable[1] == psnamemsgsend then
            ...
            if bililine == 1 then
                ...
                if nrchatmy > 0 then
                    for i = 1, #psannouncewait do
                        SendChatMessage(psannouncewait[i], "CHANNEL", nil,
                                        nrchatmy)
                        if psanounsesrazuchek == 1 then
                            psanonceinst = 1
                        end
        ...
        if (CurrentTimepull > timeeventr) then
            if (UnitIsGroupAssistant("player") or UnitIsGroupLeader("player")) then
                ...
                if psdelaydeathcheck == nil or
                    (psdelaydeathcheck and GetTime() > psdelaydeathcheck + 5) then
                ...
                for i = 1, 8 do
                    if truemark[i] == "true" then
                        ...
                        for ij = 1, #pssetmarknew[i] do
                            if metka == 0 and pssetmarknew[i][ij] and
                                ...
                                if timetocheck == 1 then
                                    ...
                                    while oj <= GetNumGroupMembers() do
                                        ...
                                        if name2 and pssetmarknew[i][ij] and
                                            string.lower(name2) ==
                                            string.lower(pssetmarknew[i][ij]) then
                                            if online == nil then
                                                psdelaydeathcheck = GetTime() -
                                                                        10
                                            end
                                            if (online and isDead == nil and
                                                UnitIsDeadOrGhost(name2) == nil) then
                                                if GetRaidTargetIndex(
                                                    pssetmarknew[i][ij]) == nil or
                                                    (GetRaidTargetIndex(
                                                        pssetmarknew[i][ij]) and
                                                        GetRaidTargetIndex(
                                                            pssetmarknew[i][ij]) ~=
                                                        i) then
                                                    SetRaidTarget(
                                    ...
                                    if UnitIsDeadOrGhost(pssetmarknew[i][ij]) ==
                                        nil then
                                        if GetRaidTargetIndex(
                                            pssetmarknew[i][ij]) == nil or
                                            (GetRaidTargetIndex(
                ...
                if (spammvar == nil) then
                    out("|cff99ffffPhoenixStyle|r - |cffff0000" .. pserror ..
                            "|r " .. psmarkserror)
                    spammvar = 1
                end
                -- отключаю
    ...
    -- мое инфо
    if psshushinfo then
        if CurrentTimepull > psshushinfo then psshushinfo = nil end
    end
...
function PhoenixStyle_OnEvent(self, event, ...)
    ...
    if event == "CHAT_MSG_ADDON" then
        ...
        if arg1 == "PhoenixStyle" and psmsgwaiting > 0 then
            ...
            if psstriniz1 == nil then
            else
                local psstrfine1 = string.find(arg2, "++", psstriniz1)
                if string.lower(string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1)) ==
                    string.lower(psmsgmychat) or
                    (string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1) == "raid" and
                        psmsgmychat == "raid_warning") or
                    (string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1) ==
                        "raid_warning" and psmsgmychat == "raid") then

                    -- вырезаем ник
                    local _, psstriniz2 = string.find(arg2, "myname:")
                    if psstriniz2 == nil then
                    else
                        ...
                        for i, getcrash in ipairs(pscanannouncetable) do
                            if getcrash ==
                                string.sub(arg2, psstriniz2 + 1, psstrfine2 - 1) then
                                psbililinet = 1
                            end
                        end
                        if (psbililinet == 0) then
                            table.insert(pscanannouncetable, string.sub(arg2,
                                                                        psstriniz2 +
                                                                            1,
                                                                        psstrfine2 -
                                                                            1))
        ...
        -- стоп аддона
        if arg1 == "PSstop" and arg2 == "stop" and
            (arg4 == "Шуршик" or arg4 == "Шурши") then
            if UnitName("player") ~= arg4 and thisaddonwork then
                out(
                    "|cff99ffffPhoenixStyle|r - " .. psaddonmy .. " |cffff0000" ..
                        psaddonoff .. "|r.")
                thisaddonwork = false
            end
        end
        ...
        if arg1 == "PhoenixStyle" and psmsgwaiting42 > 0 then
            ...
            if psstriniz1 == nil then
                ...
                local psstrfine1 = string.find(arg2, "++", psstriniz1)
                if string.lower(string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1)) ==
                    string.lower(psmsgmychat42) or
                    (string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1) == "raid" and
                        psmsgmychat42 == "raid_warning") or
                    (string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1) ==
                        "raid_warning" and psmsgmychat42 == "raid") then
                    ...
                    if psstriniz2 == nil then
                        ...
                        if (psbililinet == 0) then
                            table.insert(pscanannouncetable42, string.sub(arg2,
                                                                          psstriniz2 +
                                                                              1,
                                                                          psstrfine2 -
                                                                              1))
        ...
        if arg1 == "PhoenixStyle" and psmsgwaiting43 > 0 then
            ...
            if psstriniz1 == nil then
                ...
                local psstrfine1 = string.find(arg2, "++", psstriniz1)
                if string.lower(string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1)) ==
                    string.lower(psmsgmychat43) or
                    (string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1) == "raid" and
                        psmsgmychat43 == "raid_warning") or
                    (string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1) ==
                        "raid_warning" and psmsgmychat43 == "raid") then
                    ...
                    if psstriniz2 == nil then
                        ...
                        if (psbililinet == 0) then
                            table.insert(pscanannouncetable43, string.sub(arg2,
                                                                          psstriniz2 +
                                                                              1,
                                                                          psstrfine2 -
                                                                              1))
        ...
        if arg1 == "PhoenixStyle" then
            if string.find(arg2, "fightend") then
                ...
                local _, psstriniz1 = string.find(arg2, "mychat:")
                local psstrfine1 = string.find(arg2, "++", psstriniz1)
                psnotanonsemorechat = string.lower(string.sub(arg2,
                                                              psstriniz1 + 1,
                                                              psstrfine1 - 1))
            end
        end
        ...
        if arg1 == "PhoenixStyle" then
            local _, psstriniz1 = string.find(arg2, "mychat:")
            local psstrfine1 = string.find(arg2, "++", psstriniz1)
            if (psmsgwaiting42 == 0 or (psmsgwaiting42 > 0 and psmsgmychat42 ~=
                string.lower(string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1)))) and
                (psmsgwaiting43 == 0 or (psmsgwaiting43 > 0 and psmsgmychat43 ~=
                    string.lower(
                        string.sub(arg2, psstriniz1 + 1, psstrfine1 - 1)))) then
                if GetTime() > psnotanonsemorenormal + 2 then
                    psnotanonsemorenormal = GetTime()
                    psnotanonsemorenormalchat =
                        string.lower(string.sub(arg2, psstriniz1 + 1,
                                                psstrfine1 - 1))
                else
                    psnotanonsemorenormal2 = GetTime()
                    psnotanonsemorenormalchat2 =
                        string.lower(string.sub(arg2, psstriniz1 + 1,
                                                psstrfine1 - 1))
                end
            end
        ...
        if arg1 == "PhoenixStyle_info" and arg2 == "info" then
            ...
            if psicgalochki == nil then
                psa4 = 9
            else
                if psicgalochki[1][1] == 0 then psa4 = 0 end
            end
    ...
    if event == "PLAYER_REGEN_ENABLED" then isbattlev = 0 end

    if event == "CHAT_MSG_PARTY" or event == "CHAT_MSG_RAID" or event ==
        "CHAT_MSG_RAID_WARNING" or event == "CHAT_MSG_OFFICER" or event ==
        "CHAT_MSG_GUILD" then

        if arg1 then
            if string.lower(arg1) == "вайп" or string.lower(arg1) == "wipe" or
                string.lower(arg1) == "dfqg" or string.lower(arg1) == "цшзу" or
                string.lower(arg1) == "wipe." or string.lower(arg1) == "!wipe" or
                string.lower(arg1) == "вайпаемся" or string.lower(arg1) ==
                "вайп." or string.lower(arg1) == "ваип" then
    ...
    if GetNumGroupMembers() > 0 and event == "COMBAT_LOG_EVENT_UNFILTERED" then
        ...
        if psbilresnut == 1 then
            if pstimeresnut == nil then pstimeresnut = arg1 + 3 end
            if arg1 > pstimeresnut then
                psbilresnut = 0
                pstimeresnut = nil
            end
        end
...
function PhoenixStyleFailbot_Command(msg)
    ...
    if (string.lower(cmd) == "пулл" or string.lower(cmd) == "pull" or
        ...
        if (val == "2" or val == "3" or val == "4" or val == "5" or val == "6" or
            ...
            if (UnitIsGroupAssistant("player") or UnitIsGroupLeader("player")) then
                ...
                out(
                    "|cff99ffffPhoenixStyle|r - |cffff0000" .. pserror .. "|r " ..
                        pserrornotofficer)
        ...
        -- конец пулла
    elseif (string.lower(cmd) == "rezet" or string.lower(cmd) == "reset" or
        string.lower(cmd) == "resetpos" or string.lower(cmd) == "резет" or
        string.lower(cmd) == "сбросить" or string.lower(cmd) == "res" or
        string.lower(cmd) == "rez") then
        ...
        -- wipe
    elseif (string.lower(cmd) == "wipe" or string.lower(cmd) == "вайп" or
        ...
        -- lanatel
    elseif (string.lower(cmd) == "vampyr" or string.lower(cmd) == "vampyric" or
        string.lower(cmd) == "vampyrs" or string.lower(cmd) == "food" or
        string.lower(cmd) == "еда" or string.lower(cmd) == "еду" or
        ...
        if IsAddOnLoaded("PhoenixStyleMod_Icecrown_Classic") == false then
            ...
            if IsAddOnLoaded("PhoenixStyleMod_Icecrown_Classic") then
                if psicgalochki[1][1] == 0 then
                    ...
                    print(
                        "|cff99ffffPhoenixStyle|r - " .. psmoduleload .. " " ..
                            psleftmenu7 .. "!")
                end
            end
        end
...
function PSF_rscopen(oo)
    ...
    if rscversion and rscversion > 1.09 then
        ...
        if oo == 1 then rscwindow2() end

        if oo == 2 then rscwindow3() end
        ...
        psftxterruniq:SetText(psfneedrscaddon .. " |cffff0000" ..
                                  psfneedrscaddonver11 .. "|r\n\n" ..
                                  psraaddonanet2)
...
function PSF_potionscheck(nn)
    ...
    if rscversion then psfpotioncheckshow() end
end
...
function PSFifreporton()
    if (PSFmain3_CheckButton1:GetChecked()) then
        if (thisaddononoff) then
            ...
            thisaddononoff = true
            out("|cff99ffffPhoenixStyle|r - " .. pswarnings .. " |cff00ff00" ..
                    pswarningson .. "|r.")
        end
    else
        if (thisaddononoff) then
            out("|cff99ffffPhoenixStyle|r - " .. pswarnings .. " |cffff0000" ..
                    pswarningsoff .. "|r.")
...
function PSF_buttonsaveexit()
    if (framewasinuse2) then
        ...
        if psfautomebtable then
            for uu = 1, 8 do
                if psfautomebtable[uu]:GetText() == "" then
                    psautomarchangeno(uu)
                else
        ...
        psdatrumark = 0
        for i = 1, 8 do if truemark[i] == "true" then psdatrumark = 1 end end
        if (autorefmark and psdatrumark == 0) then
            autorefmark = false
            out(
                "|cff99ffffPhoenixStyle|r - " .. psmarkreflesh .. " |cffff0000" ..
                    psmarkrefleshoff .. "|r.")
        end
    ...
    if (framewasinuse11) then
        if (PSFmain11_primyogg:GetText() == "") then
            primyogg = ""
        else
    ...
    if frameanubused then
        pscolanubsave()
        frameanubused = nil
    end
    if framesaurfused and psiccsauractiv == nil then
        psiccsaurfsave()
        framesaurfused = nil
    end
...
function PSF_showoptions()
    if showopttime1 == nil then
        ...
        bigmenuchatlist = {
            pschatlist1, pschatlist2, pschatlist3, pschatlist4, pschatlist5,
            pschatlist6, pschatlist7, pschatlist8
        }
...
function PSFvkladdon()
    if (PSFmain3_CheckButton2:GetChecked()) then
        if (thisaddonwork) then
            ...
            thisaddonwork = true
            out("|cff99ffffPhoenixStyle|r - " .. psaddonmy .. " |cff00ff00" ..
                    psaddonon2 .. "|r.")
            pszonechangedall()
        end
    else
        if (thisaddonwork) then
            out("|cff99ffffPhoenixStyle|r - " .. psaddonmy .. " |cffff0000" ..
                    psaddonoff .. "|r.")
            thisaddonwork = false
            psmylogin = GetTime()
            if IsAddOnLoaded("PhoenixStyleMod_Icecrown_Classic") then
...
function bigmenuchat2(bigma2)
    ...
    for i, cchat in ipairs(bigmenuchatlisten) do
        if string.lower(cchat) == string.lower(bigma2) then bigma2num = i end
    end
    if bigma2num == 0 then
        for i, cchat in ipairs(psfchatadd) do
            if string.lower(cchat) == string.lower(bigma2) then
                bigma2num = i + 8
            end
        end
    end
...
function PSFprepareUI()
    bossmodtext1 = pstimersinfo2 ..
                       " |cff00ff00BigWigs    DBM    DXE    RaidWatch2|r"
end

function reportafterboitwotab(inwhichchat, bojinterr, tabln, tablc, qq, maxpers,
                              iccsv, norep, skobki)
        ...
        for i = 1, vzxnn do
            ...
            if i == vzxnn then
                if (string.len(strochkavezcrash) > 230 and qq == nil) then
                    if skobki then
                        strochkavezcrash2 =
                            strochkavezcrash2 .. tabln[i] .. pstochki
                    else
                        strochkavezcrash2 =
                            strochkavezcrash2 .. tabln[i] .. " (" .. tablc[i] ..
                                ")" .. pstochki
                    end
                else
                    if skobki then
                        strochkavezcrash =
                            strochkavezcrash .. tabln[i] .. pstochki
                    else
                        strochkavezcrash =
                            strochkavezcrash .. tabln[i] .. " (" .. tablc[i] ..
                                ")" .. pstochki
                    end
                end
                pszapuskanonsa(inwhichchat, strochkavezcrash, bojinterr, nil,
                               iccsv, norep)
                if (string.len(strochkavezcrash2) > 3 and qq == nil) then
                    pszapuskanonsa(inwhichchat, strochkavezcrash2, bojinterr,
                                   nil, iccsv, norep)
                end
            else
                if (string.len(strochkavezcrash) > 230 and qq == nil) then
...
function pscheckwipe1(numd10, numd25)
    ...
    if pswipecheckdelay == nil or
        (pswipecheckdelay and GetTime() > pswipecheckdelay + 2) then
        ...
        for i = 1, GetNumGroupMembers() do
            ...
            if ((isDead == 1 or UnitIsDeadOrGhost(nameee) == 1) and subgroup <=
                psnumgrup) then psnumdead = psnumdead + 1 end
        end

        if ((psnumgrup == 2 and psnumdead >= numd10) or
            (psnumgrup == 5 and psnumdead >= numd25)) then pswipetrue = 1 end
    ...
    if GetRealZoneText() == pszoneulduar then
        if IsAddOnLoaded("PhoenixStyleMod_Ulduar_Classic") == nil and wasuldatryload ==
            ...
            if loaded then
                ...
                print("|cff99ffffPhoenixStyle|r - " .. psmodulenotload .. " " ..
                          psleftmenu5 .. "!")
            end
        end
    end
...
function psfnotofficer()
    if (psfnotoffvar == 0) then
        psfnotoffvar = 1
        print("|cff99ffffPhoenixStyle|r - |cffff0000" .. pserror .. "|r " ..
                  psfnotofficerspam)
    end
end
...
function addtotridamagetables(nicktoadd, damagetoadd, count)
    ...
    for i, getcrash in ipairs(psdamagename) do
        if getcrash == nicktoadd then
            psdamagevalue[i] = psdamagevalue[i] + damagetoadd
            psdamageraz[i] = psdamageraz[i] + count
            bililine = 1
        end
    end
    if (bililine == 0) then
...
function addtotwodamagetables(nicktoadd, damagetoadd)
    ...
    if (psdamagevalue2 == nil or psdamagevalue2 == {}) then
        psdamagevalue2 = {}
    end
    local bililine = 0
    for i, getcrash in ipairs(psdamagename2) do
        if getcrash == nicktoadd then
            psdamagevalue2[i] = psdamagevalue2[i] + damagetoadd
            bililine = 1
        end
    end
    if (bililine == 0) then
...
function addtotwodamagetables3(nicktoadd, damagetoadd)
    ...
    if (psdamagevalue3 == nil or psdamagevalue3 == {}) then
        psdamagevalue3 = {}
    end
    local bililine = 0
    for i, getcrash in ipairs(psdamagename3) do
        if getcrash == nicktoadd then
            psdamagevalue3[i] = psdamagevalue3[i] + damagetoadd
            bililine = 1
        end
    end
    if (bililine == 0) then
...
function reportfromtridamagetables(inwhichchat, maxpers, qq, bojinterr, iccra,
                                   norep)
        ...
        for i = 1, vzxnn do
            ...
            if psdamagevalue[i] and psdamagevalue[i] > 0 then
                ...
                if (string.len(psdamagevalue[i])) > 3 then
                    psinfoshieldtempdamage =
                        string.sub(psdamagevalue[i], 1,
                                   string.len(psdamagevalue[i]) - 3)
                    psinfoshieldtempdamage = psinfoshieldtempdamage .. "k"
                else
            ...
            if i == vzxnn then
                ...
                if psdamagevalue[i] and psdamagevalue[i] > 0 then
                    if (string.len(strochkadamageout) > 230 and qq == nil) then
                        strochkavezcrash2 =
                            strochkavezcrash2 .. psdamagename[i] .. " (" ..
                                psinfoshieldtempdamage .. " - " ..
                                psdamageraz[i] .. ")" .. pstochki
                    else
                        strochkadamageout =
                            strochkadamageout .. psdamagename[i] .. " (" ..
                                psinfoshieldtempdamage .. " - " ..
                                psdamageraz[i] .. ")" .. pstochki
                    end
                end
                pszapuskanonsa(inwhichchat, strochkadamageout, bojinterr, nil,
                               iccra, norep)
...
function reportfromtwodamagetables(inwhichchat, qq, dmggood, numrrep, bojinterr,
                                   iccra, norep)
        ...
        for i = 1, vzxnn do
            ...
            if dmggood == nil or psdamagevalue2[i] > dmggood then
                ...
                if (string.len(psdamagevalue2[i])) > 3 then
                    psinfoshieldtempdamage =
                        string.sub(psdamagevalue2[i], 1,
                                   string.len(psdamagevalue2[i]) - 3)
                    psinfoshieldtempdamage = psinfoshieldtempdamage .. "k"
                else
            ...
            if i == vzxnn then
                ...
                if dmggood == nil or psdamagevalue2[i] > dmggood then
                    if (string.len(strochkadamageout) > 230 and qq == nil) then
                        strochkavezcrash2 =
                            strochkavezcrash2 .. psdamagename2[i] .. " (" ..
                                psinfoshieldtempdamage .. ")" .. pstochki
                    else
                        strochkadamageout =
                            strochkadamageout .. psdamagename2[i] .. " (" ..
                                psinfoshieldtempdamage .. ")" .. pstochki
                    end
                end
    ...
    if (vzxnn > 0) then
        ...
        for i = 1, vzxnn do
            ...
            if (string.len(psdamagevalue3[i])) > 3 then
                psinfoshieldtempdamage =
                    string.sub(psdamagevalue3[i], 1,
                               string.len(psdamagevalue3[i]) - 3)
                psinfoshieldtempdamage = psinfoshieldtempdamage .. "k"
            else
            ...
            if i == vzxnn then
                if string.len(strochkadamageout) > 230 then
                    strochkavezcrash2 = strochkavezcrash2 .. psdamagename3[i] ..
                                            " (" .. psinfoshieldtempdamage ..
                                            ")" .. pstochki
                else
                    strochkadamageout = strochkadamageout .. psdamagename3[i] ..
                                            " (" .. psinfoshieldtempdamage ..
                                            ")" .. pstochki
                end
                ...
                pszapuskanonsa(inwhichchat, strochkadamageout, bojinterr, nil,
                               iccra, norep)
                if string.len(strochkavezcrash2) > 3 then
                    pszapuskanonsa(inwhichchat, strochkavezcrash2, bojinterr,
                                   nil, iccra, norep)
                end
            else
...
function pszapuskanonsa(kudarep, chtorep, bojinterr, vajnreport, addicc, norep)
    ...
    if addicc then
        if psicclastsaverep == nil or (psicclastsaverep and
            (GetTime() < psicclastsaverep + 3 or GetTime() > psicclastsaverep +
                30)) then
            if addicc > 5 then
        ...
        if thisaddononoff and kudarep and chtorep then
            ...
            if kudarep == "sebe" then
                ...
                if ((kudarep == "raid" or kudarep == "raid_warning") and
                    ...
                    if (bojinterr and
                        ((GetTime() < psnotanonsemore + 70 and
                            psnotanonsemorechat and
                            (psnotanonsemorechat == kudarep or
                                (psnotanonsemorechat == "raid_warning" and
                                    kudarep == "raid") or
                                (psnotanonsemorechat == "raid" and kudarep ==
                                    "raid_warning"))) or GetTime() < psmylogin +
                            180)) then
                    else

                        if ((bojinterr == nil or bojinterr == false) and
                            (GetTime() < psnotanonsemorenormal + 2 and
                                psnotanonsemorenormalchat and
                                (psnotanonsemorenormalchat == kudarep or
                                    (psnotanonsemorenormalchat == "raid_warning" and
                                        kudarep == "raid") or
                                    (psnotanonsemorenormalchat == "raid" and
                                        kudarep == "raid_warning")))) then
                        else

                            if ((bojinterr == nil or bojinterr == false) and
                                (GetTime() < psnotanonsemorenormal2 + 2 and
                                    psnotanonsemorenormalchat2 and
                                    (psnotanonsemorenormalchat2 == kudarep or
                                        (psnotanonsemorenormalchat2 ==
                                            "raid_warning" and kudarep == "raid") or
                                        (psnotanonsemorenormalchat2 == "raid" and
                                            kudarep == "raid_warning")))) then
                            else
                                ...
                                if (bililine == 1 or
                                    (bililine == 0 and GetChannelName(kudarep) >
                                        0)) then
                                    ...
                                    if bojinterr then
                                        ...
                                        if psmsgwaiting == 0 then
                                            ...
                                            table.wipe(pscanannouncetable)
                                            table.insert(pscanannouncetable,
                                                         psnamemsgsend)
                                        end

                                        table.insert(psannouncewait, chtorep)

                                    elseif (psmsgwaiting42 == 0 and
                                        (psmsgwaiting43 == 0 or
                                            (psmsgwaiting43 > 0 and
                                                psmsgmychat43 ~= kudarep))) or
                                        (psmsgwaiting42 > 0 and psmsgmychat42 ==
                                            kudarep) then
                                        ...
                                        if psmsgwaiting42 == 0 then
                                            ...
                                            table.wipe(pscanannouncetable42)
                                            table.insert(pscanannouncetable42,
                                                         psnamemsgsend)
                                        end
                                        ...
                                        if psmsgwaiting43 == 0 then
                                            ...
                                            table.wipe(pscanannouncetable43)
                                            table.insert(pscanannouncetable43,
                                                         psnamemsgsend)
                                        end
...
function PS_MinimapButton_Reposition()
    PS_MinimapButton:SetPoint("TOPLEFT", "Minimap", "TOPLEFT",
                              52 - (80 * cos(PS_Settings.PSMinimapPos)),
                              (80 * sin(PS_Settings.PSMinimapPos)) - 52)
end
...
function psfautomarldraw()
    if psfautomarkdaw1 == nil then
        ...
        psfautomebtable = {}
        local psebf1 = CreateFrame("EditBox", "pseb1", PSFmain4,
                                   "InputBoxTemplate")
        pseditframecreat(psebf1, -130, 1)
        local psebf2 = CreateFrame("EditBox", "pseb2", PSFmain4,
                                   "InputBoxTemplate")
        pseditframecreat(psebf2, -155, 2)
        local psebf3 = CreateFrame("EditBox", "pseb3", PSFmain4,
                                   "InputBoxTemplate")
        pseditframecreat(psebf3, -180, 3)
        local psebf4 = CreateFrame("EditBox", "pseb4", PSFmain4,
                                   "InputBoxTemplate")
        pseditframecreat(psebf4, -205, 4)
        local psebf5 = CreateFrame("EditBox", "pseb5", PSFmain4,
                                   "InputBoxTemplate")
        pseditframecreat(psebf5, -230, 5)
        local psebf6 = CreateFrame("EditBox", "pseb6", PSFmain4,
                                   "InputBoxTemplate")
        pseditframecreat(psebf6, -255, 6)
        local psebf7 = CreateFrame("EditBox", "pseb7", PSFmain4,
                                   "InputBoxTemplate")
        pseditframecreat(psebf7, -280, 7)
        local psebf8 = CreateFrame("EditBox", "pseb8", PSFmain4,
                                   "InputBoxTemplate")
...
function pseditframecreat(a, b, nr)
    ...
    a:Show()
    a:SetScript("OnTabPressed", function(self)
        if psfautomebtable[nr + 1] then
            psfautomebtable[nr + 1]:SetFocus()
            psfautomebtable[nr + 1]:HighlightText()
        else
            psfautomebtable[1]:SetFocus()
            psfautomebtable[1]:HighlightText()
        end
    end)
    ...
    if #pssetmarknew[nr] > 0 then
        for h = 1, #pssetmarknew[nr] do
            if string.len(nicks) > 1 then nicks = nicks .. ", " end
...
function psfchatsendreports(chat, nnn1, nnn2, nnn3, nnn4, nnn5, nnn6, nnn7,
                            nnn8, nnn9, nnn10, nnn11, nnn12, whisp)
    ...
    local nnn = {
        nnn1, nnn2, nnn3, nnn4, nnn5, nnn6, nnn7, nnn8, nnn9, nnn10, nnn11,
        nnn12
    }
    for i = 1, 12 do
        if string.len(nnn[i]) > 3 then
            ...
            elseif chat == "whisper" then
                ...
                local bililine = 0
                for i, cc in ipairs(bigmenuchatlisten) do
                    if cc == chat then bililine = 1 end
                end
...
function psslidertimerefmark()
    ...
    if (autorefmark) then timeeventr = GetTime() + secrefmark end
end
