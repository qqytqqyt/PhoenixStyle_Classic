function PSF_iccsaurfang()
    if psicgalochki[1][1] == 1 then
        ...
        if psfsaurfdaw1 == nil then
            ...
            psicsaurfedittable = {}
            local psebf1 = CreateFrame("EditBox", "psebs1", PSFiccsaurf,
                                       "InputBoxTemplate")
            pseditframecreats(psebf1, -100, 1)
            local psebf2 = CreateFrame("EditBox", "psebs2", PSFiccsaurf,
                                       "InputBoxTemplate")
            pseditframecreats(psebf2, -125, 2)
            local psebf3 = CreateFrame("EditBox", "psebs3", PSFiccsaurf,
                                       "InputBoxTemplate")
            pseditframecreats(psebf3, -150, 3)
            local psebf4 = CreateFrame("EditBox", "psebs4", PSFiccsaurf,
                                       "InputBoxTemplate")
            pseditframecreats(psebf4, -175, 4)
            local psebf5 = CreateFrame("EditBox", "psebs5", PSFiccsaurf,
                                       "InputBoxTemplate")
            pseditframecreats(psebf5, -200, 5)
            local psebf6 = CreateFrame("EditBox", "psebs6", PSFiccsaurf,
                                       "InputBoxTemplate")
            pseditframecreats(psebf6, -225, 6)
            local psebf7 = CreateFrame("EditBox", "psebs7", PSFiccsaurf,
                                       "InputBoxTemplate")
            pseditframecreats(psebf7, -250, 7)
            local psebf8 = CreateFrame("EditBox", "psebs8", PSFiccsaurf,
                                       "InputBoxTemplate")
            ...
            -- чекбатоны
            for i = 1, 6 do
                local c = CreateFrame("CheckButton", nil, PSFiccsaurf,
                ...
                if psicsaurfgalk[i] == 1 then c:SetChecked(true) end
...
function pseditframecreats(a, b, nr)
    ...
    a:SetScript("OnTabPressed", function(self)
        if psicsaurfedittable[nr + 1] then
            psicsaurfedittable[nr + 1]:SetFocus()
            psicsaurfedittable[nr + 1]:HighlightText()
        else
            psicsaurfedittable[1]:SetFocus()
            psicsaurfedittable[1]:HighlightText()
        end
    end)
...
function psiccsaurfsave()
    if psiccsauractiv == nil then
        for tt = 1, 8 do psiccsv4(psicsaurfedittable[tt]:GetText(), tt) end
    end
end
...
function psiccsv4(tt, nn)
    ...
    psiccsaurf[nn] = tt
    if psiccsaurf[nn] == "" then
    else
        if psiccsauractiv and UnitInRaid(psiccsaurf[nn]) == nil then
            out("|cff99ffffPhoenixStyle|r - |cffff0000" .. psattention ..
                    "|r '|CFF00FF00" .. psiccsaurf[nn] .. "|r' - " ..
                    psnotfoundinr)
        end
    end
end
...
function psiccsaursettext8()
    for ty = 1, 8 do psicsaurfedittable[ty]:SetText(psiccsaurf[ty]) end
end
...
function PSF_iccsaursend()
    ...
    for i = 1, 8 do
        if psiccsaurf[i] == "" then
            ...
            psfchatsendreports(psicdopmodchat[1],
                               "{rt" .. i .. "} " .. psiccsaurf[i])
...
function openmenureporticcsaurf()
    if not DropDownMenureporticcsaurf then
        CreateFrame("Frame", "DropDownMenureporticcsaurf", PSFiccsaurf,
    ...
    if ((UnitIsGroupAssistant("player") or UnitIsGroupLeader("player"))) then
        ...
        for i = 1, 8 do
            if psiccsaurf[i] == "" then
                ...
                if UnitInRaid(psiccsaurf[i]) then
                    psiccschet = psiccschet + 1
                else
                    out(
                        "|cff99ffffPhoenixStyle|r - |cffff0000" .. psattention ..
                            "|r '|CFF00FF00" .. psiccsaurf[i] .. "|r' - " ..
                            psnotfoundinr)
                    psiccerr = 1
                end
            end
        ...
        if psiccerr == 0 then
            if psiccschet > 1 then
                ...
                PhoenixStyleMod_Icecrown_Classic:RegisterEvent(
                ...
                out("|cff99ffffPhoenixStyle|r - " .. psiccsaurfang ..
                        ": |cff00ff00" .. psmoduletxton .. "|r")

                -- блок изменений ников
                for tg = 1, 8 do
                    psicsaurfedittable[tg]:SetScript("OnTextChanged", function(
                        self) self:SetText(psiccsaurf[tg]) end)
                end
...
function psficcsaurfoff()
    ...
    out("|cff99ffffPhoenixStyle|r - " .. psiccsaurfang .. ": |cffff0000" ..
            psmoduletxtoff .. "|r")
...
-- фейлбот:
function psficcsaurfangnoobs(arg7)
...
function psficcsaurfangnoobscircle(arg7)
        ...
        if #vezaxname > 0 then
            if ttemp then
                ...
                if psicgalochki[5][4] == 1 then
                    reportafterboitwotab(whererepiccchat[psicchatchoose[5][4]],
                                         false, vezaxname, vezaxcrash, 1)
                end
            else
                strochkavezcrash = "{rt7} " .. psiccsaurfail1
                reportafterboitwotab(whererepiccchat[psicchatchoose[5][2]],
                                     true, vezaxname, vezaxcrash, 1)
            end
        end
    end

    if #vezaxname2 > 0 and psicgalochki[5][1] == 1 and psicgalochki[5][5] == 1 and
        ttemp == nil then
        strochkavezcrash = "{rt8} " .. psiccsaurfail2
        reportafterboitwotab(whererepiccchat[psicchatchoose[5][5]], true,
                             vezaxname2, vezaxcrash2, 1, 5)
    end

    if psicgalochki[5][1] == 1 and psicgalochki[5][7] == 1 and ttemp == nil and
        psiccbloodq then
        if psiccbloodq == 1 then
            pszapuskanonsa(whererepiccchat[psicchatchoose[5][7]],
                           "{rt3} " .. psiccanons46failtxt .. psiccbloodq, true)
        else
            pszapuskanonsa(whererepiccchat[psicchatchoose[5][7]], "{rt3} " ..
                               psiccanons46failtxt .. psiccbloodq .. " - " ..
                               psiccbloodn, true)
        end
    end

    if psiccsavfile and ttemp == nil and
        (psicclastsaverep == nil or (psicclastsaverep and
        ...
        strochkavezcrash = "{rt7} " .. psiccsaurfail1
        reportafterboitwotab(whererepiccchat[psicchatchoose[5][2]], true,
                             vezaxname, vezaxcrash, 1, 15, 0, 1)

        strochkavezcrash = "{rt8} " .. psiccsaurfail2
        reportafterboitwotab(whererepiccchat[psicchatchoose[5][5]], true,
                             vezaxname2, vezaxcrash2, 1, 5, 0, 1)
        ...
        while i < 9 do
            if psicsaurfhealstattbl[i] == 1 then
                ...
                if psicsaurfgalk[1] == 1 then
                    SendChatMessage(psiccschet5 .. " " .. psiccsaurmatka ..
                                        " >> {rt" .. i .. "}" .. im .. "! << " ..
                                        psiccsaurithael .. " - " ..
                                        psiccsaurf[i], "raid_warning")
                end

                if psicsaurfgalk[2] == 1 then
                    SendChatMessage("PhoenixStyle > " .. psiccsuarhealit ..
                                        " {rt" .. i .. "}" .. im, "WHISPER",
                                    nil, psiccsaurf[i])
                end
            ...
            while i < 9 do
                if psicsaurfhealstattbl[i] == 2 then
                    ...
                    if UnitIsDead(psiccsaurf[i]) == nil then
                        ...
                        if psicsaurfgalk[1] == 1 then
                            SendChatMessage(
                                psiccschet5 .. " " .. psiccsaurmatka ..
                                    " >> {rt" .. i .. "}" .. im .. "! << " ..
                                    psiccsaurithael .. " - " .. psiccsaurf[i],
                                "raid_warning")
                        end

                        if psicsaurfgalk[2] == 1 then
                            SendChatMessage(
                                "PhoenixStyle > " .. psiccsuarhealit .. " {rt" ..
                                    i .. "}" .. im, "WHISPER", nil,
                                psiccsaurf[i])
                        end
            ...
            if psicmetkafind == 0 then
                ...
                while i < 9 do
                    if psiccsaurf[i] == "" and psicsaufmmarkshas[i] == "" then
                        ...
                        if psicsaurfgalk[1] == 1 then
                            SendChatMessage(
                                "{rt8} " .. psiccschet5 .. " " .. psiccsaurmatka ..
                                    " >> {rt" .. i .. "}" .. im .. "! << " ..
                                    psiccsaurnohealer, "raid_warning")
                        end
                            ...
                            if psicsaurfgalk[5] == 1 then
                                SendChatMessage(
                                    "{rt8} " .. psiccschet5 .. " " ..
                                        psiccsaurmatka .. " >> {rt" .. i .. "}" ..
                                        im .. "! << " .. psiccsaurithael .. " " ..
                                        psiccsaurf[i] .. " (" .. prichina ..
                                        "). " .. psiccsaurhelp, "raid_warning")
                            end
                            SetRaidTarget(im, i)
                            psicsaufmmarkshas[i] = im
                            i = 20
                ...
                if nashelmet == 0 then
                    if psicsaurfgalk[5] == 1 then
                        SendChatMessage("{rt8} " .. psiccschet5 .. " " ..
                                            psiccsaurmatka .. " >>> " .. im ..
                                            "! <<< " .. psiccsaurerr1,
                                        "raid_warning")
                    end
...
function psicconbosssaurdead(im)
    ...
    for i = 1, 8 do
        if psiccsaurf[i] == im then
            ...
            if psicsaufmmarkshas[i] == "" or psiccsaurf[i] ==
                psicsaufmmarkshas[i] then
            else
                if psicsaurfgalk[5] == 1 then
                    if psicscetchik < 4 and psnumdead < 6 then
                        SendChatMessage(
                            im .. " " .. psiccsaurerr2 .. " >> {rt" .. i .. "}" ..
                                psicsaufmmarkshas[i] .. " <<", "raid_warning")
                    end
                end
            end
    ...
    for i = 1, 8 do
        if psicsaufmmarkshas[i] == im then
            ...
            for uu = 1, GetNumGroupMembers() do
                local name, _, _, _, _, _, _, _, isDead = GetRaidRosterInfo(uu)
                    ...
                    if psicsaurfgalk[5] == 1 then
                        if psicscetchik < 4 and psnumdead < 6 then
                            if psiccsaurf[i] == "" then
                                ...
                                local nrmetki = 0
                                if psiccsaurfmarkslog and #psiccsaurfmarkslog >
                                    0 then
    ...
    while i < 9 do
        if psicsaurfhealstattbl[i] == 1 then
            ...
            if psicsaurfgalk[3] == 1 then
                SendChatMessage(
                    psiccsaurmetkainc .. " - " .. (psiccschet5 + 1) .. ", " ..
                        psiccsaurf[i] .. " " .. psiccsaurbeready, "raid")
            end

            if psicsaurfgalk[4] == 1 then
                SendChatMessage("PhoenixStyle > " .. psiccsaurbeready2,
                                "WHISPER", nil, psiccsaurf[i])
            end
            i = 20
        end
        ...
        while i < 9 do
            if psicsaurfhealstattbl[i] == 2 then
                ...
                if UnitIsDead(psiccsaurf[i]) == nil then
                    ...
                    if psicsaurfgalk[3] == 1 then
                        SendChatMessage(psiccsaurmetkainc .. " - " ..
                                            (psiccschet5 + 1) .. ", " ..
                                            psiccsaurf[i] .. " " ..
                                            psiccsaurbeready, "raid")
                    end
    ...
    for i = 1, GetNumGroupMembers() do
        ...
        for j = 1, 8 do
            if psiccsaurf[j] == psnname then
                if (pssonline == nil and
                    (psicsaurfhealstattbl[j] == 1 or psicsaurfhealstattbl[j] ==
                        2 or psicsaurfhealstattbl[j] == 3)) then
...
function psiccsauredcclfocus()
    for ii = 1, 8 do psicsaurfedittable[ii]:ClearFocus() end
end
