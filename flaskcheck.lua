function rsccreateframeflasks()
    if rsccreateframebafsvar2 == nil then
        ...
        rscmaintxt1:SetFont(GameFontNormal:GetFont(), 11)
        rscmaintxt1:SetText(rscflasktext1)
        rscmaintxt1:SetWidth(555)
        ...
        rscflaskout:SetFont(GameFontNormal:GetFont(), 11)
        rscflaskout:SetText(" ")
        rscflaskout:SetWidth(395)
        rscflaskout:SetHeight(155)
        ...
        rscflaskout2:SetJustifyH("LEFT")
        rscflaskout2:SetJustifyV("TOP")
        rscflaskout2:SetPoint("TOPLEFT", 86, -367)

        -- txt autocheck
        local tt2 = rscframetoshowall4:CreateFontString()
        tt2:SetFont(GameFontNormal:GetFont(), 11)
        tt2:SetText(rscflasktext2)
        -- tt2:SetWidth(555)
        -- tt2:SetHeight(60)
        tt2:SetJustifyH("LEFT")
        tt2:SetJustifyV("TOP")
        tt2:SetPoint("TOPLEFT", 405, -67) -- 82

        -- txt not in some zones
        local tt3 = rscframetoshowall4:CreateFontString()
        tt3:SetFont(GameFontNormal:GetFont(), 13)
        tt3:SetText("|cffff0000" .. rscaddonnotworkinz .. "|r")
        -- tt2:SetWidth(555)
        -- tt2:SetHeight(60)
        tt3:SetJustifyH("LEFT")
        tt3:SetJustifyV("TOP")
        tt3:SetPoint("TOPLEFT", 325, -116)

        if psversion then
            PSFrscflask_Button1:SetPoint("TOPLEFT", 20, -220)
            PSFrscflask_Button1:SetText(rscreloadbutton)
            PSFrscflask_Button2:SetPoint("TOPLEFT", 20, -310)
            ...
            rscmain4_Button1:SetPoint("TOPLEFT", 20, -220)
            rscmain4_Button2:SetPoint("TOPLEFT", 20, -310)
        ...
        t1:SetJustifyH("LEFT")
        t1:SetPoint("TOPLEFT", 43, -77)

        local t2 = rscframetoshowall4:CreateFontString()
        t2:SetFont(GameFontNormal:GetFont(), 11)
        t2:SetText(rscflasktextc12)
        t2:SetJustifyH("LEFT")
        t2:SetPoint("TOPLEFT", 43, -97)

        local t3 = rscframetoshowall4:CreateFontString()
        t3:SetFont(GameFontNormal:GetFont(), 11)
        t3:SetText(rscflasktextc13)
        t3:SetJustifyH("LEFT")
        t3:SetPoint("TOPLEFT", 43, -117)

        local t4 = rscframetoshowall4:CreateFontString()
        t4:SetFont(GameFontNormal:GetFont(), 11)
        t4:SetText(rscflasktextc14)
        t4:SetJustifyH("LEFT")
        t4:SetPoint("TOPLEFT", 43, -137)

        local t5 = rscframetoshowall4:CreateFontString()
        t5:SetFont(GameFontNormal:GetFont(), 11)
        t5:SetText(rscpartanons35)
        t5:SetJustifyH("LEFT")
        t5:SetPoint("TOPLEFT", 43, -157)

        rscflaskt6 = rscframetoshowall4:CreateFontString()
        rscflaskt6:SetFont(GameFontNormal:GetFont(), 11)
        ...
        rscflaskt6:SetPoint("TOPLEFT", 43, -177)

        rscflaskt7 = rscframetoshowall4:CreateFontString()
        rscflaskt7:SetFont(GameFontNormal:GetFont(), 11)
        ...
        rscflaskt7:SetPoint("TOPLEFT", 43, -197)
...
function rscsliderflaskchan()
    ...
    rscflaskout2:SetText("|cffff0000" .. rscminflaskgood .. "|r")
    if rscminflaskgood == 0 then
        rscmaintxt1:SetText(rscflasktext1)
    else
        rscmaintxt1:SetText(rscflasktext1 .. " " ..
                                format(rscflasktext1part2, rscminflaskgood))
    end
...
function openmenuflask1()
    ...
    if not DropDownMenuflask1 then
        CreateFrame("Frame", "DropDownMenuflask1", rscframetoshowall4,
                    "UIDropDownMenuTemplate")
    end

    DropDownMenuflask1:ClearAllPoints()
    DropDownMenuflask1:SetPoint("TOPLEFT", 385, -83) -- 98
...
function openmenuflask3()
    ...
    if not DropDownMenuflask3 then
        CreateFrame("Frame", "DropDownMenuflask3", rscframetoshowall4,
                    "UIDropDownMenuTemplate")
    end

    DropDownMenuflask3:ClearAllPoints()
    DropDownMenuflask3:SetPoint("TOPLEFT", 14, -280)
    ...
    if GetNumRaidMembers() > 0 then
        ...
        if #rscwillcheckthem > 0 and rscflaskcheckb[6] == 1 then
            for yy = 1, #rscwillcheckthem do
                ...
                for uu = 1, #rscfoodtable do
                    if UnitAura(rscwillcheckthem[yy],
                        ...
                        local _, _, _, _, _, _, expirationTime = UnitAura(
                                                                     ...
                                                                     GetSpellInfo(
                                                                         rscfoodtable[uu]))
                        if expirationTime == nil or
                            (expirationTime and expirationTime - GetTime() >
                                rscminflaskgood * 60) or expirationTime == 0 then
                            byl = 100
                            uu = 100
                        end
                        if expirationTime and expirationTime - GetTime() <=
                            rscminflaskgood * 60 and expirationTime - GetTime() >
                            60 then
                ...
                elseif byl ~= 100 then
                    table.insert(rscnofoodbuff,
                                 rscwillcheckthem[yy] .. " (" .. byl .. ")")
                end
            end
        end
        ...
        if #rscelixtabl2 > 0 then
            for gg = 1, #rscelixtabl2 do
                if rscelixtabl2[gg] and
                    (rscelixtabl2[gg] == 2 or rscelixtabl2[gg] == 1) then
        ...
        if rscflaskout and rscframetoshowall4:IsShown() then
            local stringa = ""
            local slak = 0
            local sringamode = rscflasktext3 .. " " .. rscgropcheck .. " " ..
                                   rscflasktxtgroup2 .. "."
            if rscgropcheck == 5 then
                sringamode = rscflasktext3 .. " " .. rscgropcheck .. " " ..
                                 rscflasktxtgroup5 .. "."
            end
            ...
            if rscflaskcheckb[7] == 1 then
                if #rsconlyoneelixir > 0 then
                    stringa =
                        stringa .. "|cffff0000" .. rscflasktext4 .. "|r (" ..
                            #rsconlyoneelixir .. "): "
                    for eda = 1, #rsconlyoneelixir do
                        ...
                        stringa =
                            stringa .. rsccolor1 .. rsconlyoneelixir[eda] ..
                                rsccolor2
                    end
                    stringa = stringa .. "\r\n"
                end
            ...
            if #rscnoflask > 0 then
                ...
                if #rscnoflask == #rscwillcheckthem and #rscnoflask > 6 then
                    stringa =
                        stringa .. "|cffff0000" .. rscflasktext5 .. "|r (" ..
                            #rscnoflask .. "): " .. string.upper(rscflasktext6) ..
                            "!\r\n"
                else
                    stringa =
                        stringa .. "|cffff0000" .. rscflasktext5 .. "|r (" ..
                    ...
                    for eda = 1, #rscnoflask do
                        ...
                        if string.find(rscnoflask[eda], "%(") then
                            rname = string.sub(rscnoflask[eda], 1, string.find(
                                                   rscnoflask[eda], "%(") - 2)
                            rname2 = string.sub(rscnoflask[eda], string.find(
                                                    rscnoflask[eda], "%(") - 1)
                        else
                            rname = rscnoflask[eda]
                        end

                        stringa = stringa .. rsccolor1 .. rname .. rsccolor2 ..
                                      rname2
                    end
                    stringa = stringa .. "\r\n"
                end
            ...
            if #rscwillcheckthem == #rscnofoodbuff and #rscwillcheckthem > 6 then
                -- ни у кого нет еды
                stringa = stringa .. "|cffff0000" .. rscflasktext7 .. "|r (" ..
                              #rscnofoodbuff .. "): " .. rscflasktext6 ..
                              "!\r\n"

            elseif #rscwillcheckthem - #rscnofoodbuff <= 3 and #rscwillcheckthem >
                6 then
                -- зеленым есть еда
                if #rscnofoodbuff > 0 and rscflaskcheckb[6] == 1 then
                    ...
                    for jj = 1, #rscnofoodbuff do
                        for kk = 1, #ukogoeda do
                            if ukogoeda[kk] and ukogoeda[kk] ==
                                rscnofoodbuff[jj] then
                    ...
                    stringa =
                        stringa .. "|cff00ff00" .. rscflasktext8 .. "|r (" ..
                            #ukogoeda .. "): "
                    for eda = 1, #ukogoeda do
                        ...
                        stringa = stringa .. rsccolor1 .. ukogoeda[eda] ..
                                      rsccolor2
                    end
                    stringa = stringa .. "\r\n"
                end
                ...
                if #rscnofoodbuff > 0 and rscflaskcheckb[6] == 1 then
                    stringa =
                        stringa .. "|cffff0000" .. rscflasktext7 .. "|r (" ..
                            #rscnofoodbuff .. "): "
                    for eda = 1, #rscnofoodbuff do
                        ...
                        if string.find(rscnofoodbuff[eda], "%(") then
                            rname = string.sub(rscnofoodbuff[eda], 1,
                                               string.find(rscnofoodbuff[eda],
                                                           "%(") - 2)
                            rname2 = string.sub(rscnofoodbuff[eda], string.find(
                                                    rscnofoodbuff[eda], "%(") -
                                                    1)
                        else
            ...
            -- был ли слак
            if string.len(stringa) > 2 then slak = 1 end

            -- кого не проверяли
            if #rscwillnotbechecked > 0 then
                stringa = stringa .. "|cffff0000" .. rscflasktext9 .. "|r (" ..
                              #rscwillnotbechecked .. "): "
                for eda = 1, #rscwillnotbechecked do
                    if eda > 1 then stringa = stringa .. ", " end
                    rsccoloraddfunc(rscwillnotbechecked[eda])
                    stringa =
                        stringa .. rsccolor1 .. rscwillnotbechecked[eda] ..
                            rsccolor2
                end
            end
        ...
        if rscflaskout then rscflaskout:SetText(" ") end
    end
end
    ...
    if GetNumRaidMembers() > 0 then
        ...
        if #rscwillcheckthem == #rscnofoodbuff and #rscwillcheckthem > 6 then
            ...
            stringa3 = stringa3 .. rscflasktext7 .. " (" .. #rscnofoodbuff ..
                           "): " .. rscflasktext6 .. "!"
        elseif #rscwillcheckthem - #rscnofoodbuff <= 3 and #rscwillcheckthem > 6 then
            -- есть еда
            if #rscnofoodbuff > 0 and rscflaskcheckb[6] == 1 then
                local ukogoeda = {}
                for gv = 1, #rscwillcheckthem do
            ...
            if #rscnofoodbuff > 0 and rscflaskcheckb[6] == 1 then
                stringa3 =
                    stringa3 .. rscflasktext7 .. " (" .. #rscnofoodbuff .. "): "
                for eda = 1, #rscnofoodbuff do
        ...
        if rscflaskcheckb[7] == 1 then
            if #rsconlyoneelixir > 0 then
                stringa1 = stringa1 .. rscflasktext4 .. " (" ..
                               #rsconlyoneelixir .. "): "
                for eda = 1, #rsconlyoneelixir do
                    -- whisper add
                    if #whisper1 > 0 then
        ...
        if #rscnoflask > 0 then
            ...
            if #rscnoflask == #rscwillcheckthem and #rscnoflask > 6 then
                stringa1 = stringa1 .. rscflasktext5 .. " (" .. #rscnoflask ..
                               "): " .. string.upper(rscflasktext6) .. "!"
            else
        ...
        -- был ли слак
        if string.len(stringa1) > 2 or string.len(stringa2) > 2 or
            string.len(stringa3) > 2 then slak = 1 end

        -- кого не проверяли
        if #rscwillnotbechecked > 0 then
            stringa4 =
                stringa4 .. rscflasktext9 .. " (" .. #rscwillnotbechecked ..
                    "): "
            for eda = 1, #rscwillnotbechecked do
        ...
        if manual == "manual" or (manual == "auto" and
            ...
            local canreport = 0
            if manual == "manual" then canreport = 1 end
            ...
            if rscflaskchat[1] == "raid" or rscflaskchat[1] == "raid_warning" then
                ...
                if chatfound == 0 then byyyl = 1 end
            end

            if byyyl == 1 then canreport = 1 end
        ...
        if rscflaskcheckb[4] == 1 and GetTime() - rscflaskimportchat2[1] > 100 and
            ...
            for i = 1, #whisper1 do
                ...
                if whisper2[i] == 1 then
                    SendChatMessage(rscflaskwhisptxt1, "WHISPER", nil, name2)
                end
                ...
                if whisper2[i] == 11 then
                    SendChatMessage(rscflaskwhisptxt11, "WHISPER", nil, name2)
                end
    ...
    if name then
        ...
        if rsccolornick and klass then
            ...
            if klass == "warrior" then
                rsccolor1 = "|CFFC69B6D"
            elseif klass == "deathknight" then
                rsccolor1 = "|CFFC41F3B"
            elseif klass == "paladin" then
                rsccolor1 = "|CFFF48CBA"
            elseif klass == "priest" then
                rsccolor1 = "|CFFFFFFFF"
            elseif klass == "shaman" then
                rsccolor1 = "|CFF1a3caa"
            elseif klass == "druid" then
                rsccolor1 = "|CFFFF7C0A"
            elseif klass == "rogue" then
                rsccolor1 = "|CFFFFF468"
            elseif klass == "mage" then
                rsccolor1 = "|CFF68CCEF"
            elseif klass == "warlock" then
                rsccolor1 = "|CFF9382C9"
            elseif klass == "hunter" then
                rsccolor1 = "|CFFAAD372"
            end
            rsccolor2 = "|r"
...
function rsczoneflaskch(nn)
    ...
    if nn == 1 then
        local rrr = 0
        if #rscignorzone > 0 then
            for i = 1, #rscignorzone do
                if rscignorzone[i] == GetRealZoneText() then
                    rrr = 1
                end
            end
        end
        if rrr == 0 then
            table.insert(rscignorzone, GetRealZoneText())
            out("|cff99ffffRaidSlackCheck|r - " .. rsczonereport2 ..
                    "|cff00ff00" .. GetRealZoneText() .. "|r")
        else
    ...
    if nn == 2 then
        ...
        if #rscignorzone > 0 then
            for i = 1, #rscignorzone do
                if rscignorzone[i] == GetRealZoneText() then
                    rrr = 1
                    out("|cff99ffffRaidSlackCheck|r - " .. rsczonereport3 ..
                            "|cff00ff00" .. GetRealZoneText() .. "|r")
                    table.remove(rscignorzone, i)
                    i = 1000
                end
            end
        end
    ...
    if nn == 3 then
        local txt = ""
        if #rscignorzone > 0 then
            table.sort(rscignorzone)
            for i = 1, #rscignorzone do
                txt = txt .. "|cffff0000" .. rscignorzone[i] .. "|r"
                if #rscignorzone > i then txt = txt .. ", " end
            end
        end
        if #rscignorzone == 0 then
            out("|cff99ffffRaidSlackCheck|r - " .. rsczonereport1 ..
                    "|cffff0000" .. rsczonereport6 .. "|r")
        else
    ...
    if nn == 4 then
        if rscquickclicktimer and GetTime() < rscquickclicktimer + 10 then
            -- удаляю
            out("|cff99ffffRaidSlackCheck|r - |cff00ff00 " .. rsczonereport5 ..
                    "|r")
            table.wipe(rscignorzone)
        else
            -- тыкни еще раз!
