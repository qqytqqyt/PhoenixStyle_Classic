...
function psiccaftcombop()
    ...
    if psflanadrawinf == nil then
        ...
        psiccinfscroll = CreateFrame("ScrollFrame", "psiccinfscroll",
                                     PSFiccaftercombinfoframe,
                                     "UIPanelScrollFrameTemplate")
        psiccinfscroll:SetPoint("TOPLEFT", PSFiccaftercombinfoframe, "TOPLEFT",
                                20, -60)
        ...
        psiccinfframe:SetPoint("BOTTOMRIGHT", psiccinfscroll, "BOTTOMRIGHT", 0,
                               0)
        ...
        PSFiccaftercombinfoframe_edbox2:SetScript("OnTextChanged",
                                                  function(self)
            if string.len(PSFiccaftercombinfoframe_edbox2:GetText()) > 1 then
                PSFiccaftercombinfoframe_Button2:Show()
...
function openiccbosschsv()
    if not DropDowniccbosschsv then
        CreateFrame("Frame", "DropDowniccbosschsv", PSFiccaftercombinfoframe,
    ...
    if #psiccsavedfails[1] == 0 then
        ...
        for j = 1, #psiccsavedfails[1] do
            ...
            if string.find(psiccsavedfails[2][j], text) then
                items[j] = psiccsavedfails[1][j] .. slojn .. ", " ..
                               string.sub(psiccsavedfails[2][j], 1, string.find(
                                              psiccsavedfails[2][j], ",") - 1)
            else
                items[j] = psiccsavedfails[1][j] .. slojn .. ", " ..
                               psiccsavedfails[2][j]
            end
    ...
    if psiccsv1 == nil then psiccsv1 = 1 end
...
function openicclogfailchat()
    if not DropDownicclogfailchat then
        CreateFrame("Frame", "DropDownicclogfailchat", PSFiccaftercombinfoframe,
                    "UIDropDownMenuTemplate")
    end
                    ...
                    if psiccsavedfails[i][nr] ~= "0" then
                        if (string.find(psiccsavedfails[i][nr], "{") == 1) then
                            if (string.find(psiccsavedfails[i][nr], "} ")) then
                                text = text .. "- " ..
                                           string.sub(psiccsavedfails[i][nr],
                                                      string.find(
                                                          psiccsavedfails[i][nr],
                                                          "} ") + 2) .. "\r\n"
                            else
                                text = text .. "- " ..
                                           string.sub(psiccsavedfails[i][nr],
...
function psiccsavedinforeport(nn)
    ...
    if #psiccsavedfails[1] > 0 then
        ...
        if string.find(psiccsavedfails[2][psiccsv1], text) then
            txtboss =
                txtboss .. psiccsavedfails[1][psiccsv1] .. slojn .. ", " ..
                    string.sub(psiccsavedfails[2][psiccsv1], 1, string.find(
                                   psiccsavedfails[2][psiccsv1], ",") - 1)
        else
            txtboss =
                txtboss .. psiccsavedfails[1][psiccsv1] .. slojn .. ", " ..
                    psiccsavedfails[2][psiccsv1]
        end

        if nn == 1 then
            psfchatsendreports(psicdopmodchat[3], txtboss,
                               psiccsavedfails[5][psiccsv1],
                               psiccsavedfails[6][psiccsv1],
                               psiccsavedfails[7][psiccsv1],
                               psiccsavedfails[8][psiccsv1],
                               psiccsavedfails[9][psiccsv1],
                               psiccsavedfails[10][psiccsv1],
                               psiccsavedfails[11][psiccsv1],
                               psiccsavedfails[12][psiccsv1],
                               psiccsavedfails[13][psiccsv1],
                               psiccsavedfails[14][psiccsv1],
                               psiccsavedfails[15][psiccsv1])
        end
        if nn == 2 then
            psfchatsendreports("whisper", txtboss, psiccsavedfails[5][psiccsv1],
                               psiccsavedfails[6][psiccsv1],
                               psiccsavedfails[7][psiccsv1],
                               psiccsavedfails[8][psiccsv1],
                               psiccsavedfails[9][psiccsv1],
                               ...
                               psiccsavedfails[14][psiccsv1],
                               psiccsavedfails[15][psiccsv1],
                               PSFiccaftercombinfoframe_edbox2:GetText())
        end
...
function pssvinfoalltog(onlytoday)
    if #psiccsavedfails[1] > 0 then
        ...
        for i = 1, #psiccsavedfails[1] do
            ...
            if onlytoday == nil or
                (onlytoday and string.find(psiccsavedfails[2][i], text)) then
                ...
                if string.find(psiccsavedfails[2][i], text) then
                    txttoshow = txttoshow .. psiccsavedfails[1][i] .. slojn ..
                                    ", " ..
                                    string.sub(psiccsavedfails[2][i], 1,
                                               string.find(
                                                   psiccsavedfails[2][i], ",") -
                                                   1)
                else
                ...
                for u = 5, 15 do
                    if psiccsavedfails[u][i] and
                        string.len(psiccsavedfails[u][i]) > 6 then
                        somethingadded = 1
                        if (string.find(psiccsavedfails[u][i], "{") == 1) then
                            if (string.find(psiccsavedfails[u][i], "} ")) then
                                txttoshow =
                                    txttoshow .. "- " ..
                                        string.sub(psiccsavedfails[u][i],
                                                   string.find(
                                                       psiccsavedfails[u][i],
                                                       "} ") + 2) .. "\r\n"
                            else
