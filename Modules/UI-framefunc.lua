function PSF_buttonoffmark()
    ...
    if (autorefmark) then
        autorefmark = false
        out("|cff99ffffPhoenixStyle|r - " .. psmarkreflesh .. " |cffff0000" ..
                psmarkrefleshoff .. "|r.")
    end
    psautomarunfoc()
    for i = 1, 8 do psautoupok[i]:SetText("|cffff0000-|r") end
end

function PSF_buttonresetmark()
    for i = 1, 8 do
        for j = 1, #pssetmarknew[i] do
            if pssetmarknew[i][j] and UnitExists(pssetmarknew[i][j]) and
                GetRaidTargetIndex(pssetmarknew[i][j]) then
        ...
        if (IsRaidOfficer() == 1) then
            ...
            if (psdatrumark == 1) then
                if (IsRaidOfficer() == 1) then
                    ...
                    out("|cff99ffffPhoenixStyle|r - " .. psmarkreflesh ..
                            " |cff00ff00" .. psmarkrefleshon .. "|r.")
                else
...
function psmarksisinraid(chekat, trueli, nomir)
    if trueli == 1 then
        ...
        while string.len(rsctemptext) > 1 do
            if (string.find(rsctemptext, " ") and string.find(rsctemptext, " ") ==
                1) or
                (string.find(rsctemptext, "%.") and
                    string.find(rsctemptext, "%.") == 1) or
                (string.find(rsctemptext, ",") and string.find(rsctemptext, ",") ==
                    1) then
        ...
        if spisoknikov[1] then
            ...
            if UnitInRaid(spisoknikov[1]) then
                ...
                if UnitExists(pssetmarknew[nomir][1]) and
                    (GetRaidTargetIndex(pssetmarknew[nomir][1]) == nil or
                        (GetRaidTargetIndex(pssetmarknew[nomir][1]) and
                            GetRaidTargetIndex(pssetmarknew[nomir][1]) ~= nomir)) then
                    ...
                    for uy = 2, #spisoknikov do
                        if UnitInRaid(spisoknikov[uy]) then
                            ...
                            out("|cff99ffffPhoenixStyle|r - '|CFF00FF00" ..
                                    spisoknikov[uy] .. "|r' - " .. psnotfoundinr)
                            psautoupok[nomir]:SetText(
                                "|cff00ff00ok|r |cffff0000-|r")
                            table.insert(pssetmarknew[nomir], spisoknikov[uy])
                        end
                    end
                end
                ...
                out("|cff99ffffPhoenixStyle|r - |cffff0000" .. psattention ..
                        "|r '|CFF00FF00" .. spisoknikov[1] .. "|r' - " ..
                        psnotfoundinr .. " " .. format(psautomarnotu, nomir))
            ...
            if UnitInRaid(spisoknikov[1]) then
                ...
                if autorefmark then
                    ...
                    if UnitExists(pssetmarknew[nomir][1]) and
                        (GetRaidTargetIndex(pssetmarknew[nomir][1]) == nil or
                            (GetRaidTargetIndex(pssetmarknew[nomir][1]) and
                                GetRaidTargetIndex(pssetmarknew[nomir][1]) ~=
                                nomir)) then
                        SetRaidTarget(pssetmarknew[nomir][1], nomir)
                    end
                end
...
function openremovechat()
    if not DropDownremovechat then
        CreateFrame("Frame", "DropDownremovechat", PSFmainchated,
...
function openaddchat()
    if not DropDownaddchat then
        CreateFrame("Frame", "DropDownaddchat", PSFmainchated,
                    "UIDropDownMenuTemplate")
    end
    ...
    local pschc2 = {}
    table.wipe(pschc2)
    pschc2[1], pschc2[2], pschc2[3], pschc2[4], pschc2[5], pschc2[6], pschc2[7], pschc2[8], pschc2[9], pschc2[10] =
        EnumerateServerChannels()
    if #pschc2 > 0 then
        for i = 1, #pschc2 do
            for j = 1, #pschc do
                if pschc[j] == pschc2[i] then
...
function PSF_addchatrep()
    if #psfchatadd > 4 then
        ...
        out("|cff99ffffPhoenixStyle|r - |cff00ff00" .. pschc[pschcnum] ..
                "|r - " .. pschataddsuc)
        openremovechat()
        openaddchat()
    end
...
function PSF_remchatrep()
    if pschcnumrem > 8 then
        out("|cff99ffffPhoenixStyle|r - |cffff0000" ..
                bigmenuchatlist[pschcnumrem] .. "|r - " .. pschatremsuc)
...
function psautomarunfoc() for tg = 1, 8 do psfautomebtable[tg]:ClearFocus() end end
