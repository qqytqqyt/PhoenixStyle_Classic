function rsccreateframebafs()
    if rsccreateframebafsvar == nil then
        ...
        local spisokid = {
            25898, 48470, 48162, 48073, 48169, 43002, 48938, 48934, 43017
        }
        ...
        local rscebf1 = CreateFrame("EditBox", "rsceb1", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf1, -205, 1)
        local rscebf2 = CreateFrame("EditBox", "rsceb2", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf2, -225, 2)
        local rscebf3 = CreateFrame("EditBox", "rsceb3", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf3, -245, 3)
        local rscebf4 = CreateFrame("EditBox", "rsceb4", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf4, -265, 4)
        local rscebf5 = CreateFrame("EditBox", "rsceb5", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf5, -285, 5)
        local rscebf6 = CreateFrame("EditBox", "rsceb6", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf6, -305, 6)
        local rscebf7 = CreateFrame("EditBox", "rsceb7", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf7, -325, 7)
        local rscebf8 = CreateFrame("EditBox", "rsceb8", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf8, -345, 8)
        local rscebf9 = CreateFrame("EditBox", "rsceb9", rscframetoshowall5,
                                    "InputBoxTemplate")
        rsceditframecreat(rscebf9, -365, 9)

        -- txt main
        rsctxttitlemain = rscframetoshowall5:CreateFontString()
        rsctxttitlemain:SetFont(GameFontNormal:GetFont(), 11)
        rsctxttitlemain:SetText(rscbuffspart1 .. " |cff00ff00" ..
                                    rscbufftimers[1] .. " " .. rscsec .. "|r " ..
                                    rscbuffspart2 .. " |cff00ff00" ..
                                    rscbufftimers[2] .. " " .. rscsec .. "|r " ..
                                    rscbuffspart3)
        rsctxttitlemain:SetWidth(555)
        ...
        tt:SetFont(GameFontNormal:GetFont(), 12)
        tt:SetText("|cff00ff00" .. rscpartwhobuff1 .. "|r")
        tt:SetWidth(570)
        tt:SetHeight(20)
        tt:SetJustifyH("LEFT")
        tt:SetPoint("TOPLEFT", 15, -187)
        ...
        -- 1 add checkbox
        local rsccheckbox1 = CreateFrame("CheckButton", nil, rscframetoshowall5,
                                         "OptionsCheckButtonTemplate")
        rsccheckbox1:SetWidth("25")
        rsccheckbox1:SetHeight("25")
        ...
        rsctextaddnon:SetFont(GameFontNormal:GetFont(), 12)
        if rscbuffcheckb[1] == 1 then
        ...
        for i = 1, 9 do
            ...
            -- checkbuttons
            local c = CreateFrame("CheckButton", nil, rscframetoshowall5,
                                  "OptionsCheckButtonTemplate")
            c:SetWidth("25")
            c:SetHeight("25")
            c:SetPoint("TOPLEFT", 220, -183 - i * 20)
            c:SetScript("OnClick", function(self)
                rsccheckbutchanged(i)
            end)
...
function rsceditframecreat(a, b, nr)
    ...
    a:SetScript("OnTabPressed", function(self)
        rscsavenicks3()
        if rsceditboxtable[nr + 1] then
            rsceditboxtable[nr + 1]:SetFocus()
            rsceditboxtable[nr + 1]:HighlightText()
        else
            rsceditboxtable[1]:SetFocus()
            rsceditboxtable[1]:HighlightText()
        end
    end)
    a:SetScript("OnEnterPressed", function(self)
        rscsavenicks3()
        rsceditboxtable[nr]:ClearFocus()
    end)
end

function rsccheckbutchanged(nr)
    if rscbuffwhichtrack[nr] == 1 then
        rscbuffwhichtrack[nr] = 0
    else
...
function openmenubuff1()
    ...
    if not DropDownMenubuff1 then
        CreateFrame("Frame", "DropDownMenubuff1", rscframetoshowall5,
...
function openmenubuff2()
    ...
    if not DropDownMenubuff2 then
        CreateFrame("Frame", "DropDownMenubuff2", rscframetoshowall5,
                    "UIDropDownMenuTemplate")
    end
...
function rscslider51()
    ...
    rsctxttitlemain:SetText(
        rscbuffspart1 .. " |cff00ff00" .. rscbufftimers[1] .. " " .. rscsec ..
            "|r " .. rscbuffspart2 .. " |cff00ff00" .. rscbufftimers[2] .. " " ..
            rscsec .. "|r " .. rscbuffspart3)
end
...
function rscsavenicks3()
    ...
    for q = 1, 9 do
        local rsctemptext2 = ""
        if rsceditboxtable[q]:GetText() and
            string.len(rsceditboxtable[q]:GetText()) > 1 then
            local rsctemptext = rsceditboxtable[q]:GetText() .. " "
            while string.len(rsctemptext) > 1 do
                if (string.find(rsctemptext, " ") and
                    string.find(rsctemptext, " ") == 1) or
                    (string.find(rsctemptext, "%.") and
                        string.find(rsctemptext, "%.") == 1) or
                    (string.find(rsctemptext, ",") and
                        string.find(rsctemptext, ",") == 1) then
                    ...
                    local min = 1000
                    if a1 and a1 < min then min = a1 end
                    if a2 and a2 < min then min = a2 end
                    if a3 and a3 < min then min = a3 end
                    if min == 1000 then
                        rsctemptext = ""
                    else
                        ...
                        rsctemptext2 = rsctemptext2 ..
                                           string.sub(rsctemptext, 1, min - 1)
...
function rscshownicks3()
    ...
    if rsceditboxtable then
        for w = 1, 9 do rsceditboxtable[w]:SetText(rscwhomustbufflist[w]) end
    end
end
    ...
    if rscwhomustbufflist[nrbuf] and string.len(rscwhomustbufflist[nrbuf]) > 1 then
        ...
        while string.len(mystring) > 1 do
            if string.find(mystring, ", ") then
                table.insert(nicktable, string.sub(mystring, 1, string.find(
                                                       mystring, ", ") - 1))
        ...
        if #nicktable > 0 then
            for yyy = 1, #nicktable do
                if UnitExists(nicktable[yyy]) then
                    local _, englishClass = UnitClass(nicktable[yyy])
                    if UnitInRaid(nicktable[yyy]) and
                        UnitIsDeadOrGhost(nicktable[yyy]) == nil and
                        UnitAffectingCombat(nicktable[yyy]) and
                        (classs == nil or
                            (classs and englishClass and classs ~= englishClass)) then
                        rscchatfiltimefunc()
                        if classs then
                            if UnitSex(whobuff) and UnitSex(whobuff) == 3 then
                                SendChatMessage(
                                    "{rt4}" .. whobuff .. " - " .. time .. " " ..
                                        rscsec .. ", " .. rscreleasedtxt6f ..
                                        ": " .. whatbuf .. "!", "WHISPER", nil,
                                    nicktable[yyy])
                            else
                                SendChatMessage(
                                    "{rt4}" .. whobuff .. " - " .. time .. " " ..
                                        rscsec .. ", " .. rscreleasedtxt6 ..
                                        ": " .. whatbuf .. "!", "WHISPER", nil,
                                    nicktable[yyy])
                            end
                            rscprioritytemp = 1
                            ...
                            if UnitSex(whobuff) and UnitSex(whobuff) == 3 then
                                SendChatMessage(
                                    "{rt3}" .. whobuff .. " " ..
                                        rscreleasedtxt1f .. " " .. time .. " " ..
                                        rscsec .. " " .. rscreleasedtxt2 .. ", " ..
                                        rscreleasedtxt7f .. ": " .. whatbuf ..
                                        "!", "WHISPER", nil, nicktable[yyy])
                            else
                                SendChatMessage(
                                    "{rt3}" .. whobuff .. " " .. rscreleasedtxt1 ..
                                        " " .. time .. " " .. rscsec .. " " ..
                                        rscreleasedtxt2 .. ", " ..
                                        rscreleasedtxt7 .. ": " .. whatbuf ..
                                        "!", "WHISPER", nil, nicktable[yyy])
                            end
                            rscprioritytemp = 1
                        end
...
function rscreportchatbyname(name1, name2, time, buff, nr)
    ...
    if nr == 1 then
        if UnitSex(name1) and UnitSex(name1) == 3 then
            SendChatMessage(
                "{rt3}" .. name1 .. " " .. rscreleasedtxt1f .. " " .. time ..
                    " " .. rscsec .. " " .. rscreleasedtxt2 .. ", " ..
                    rscreleasedtxt7f .. ": " .. buff .. "!", "WHISPER", nil,
                name2)
        else
            SendChatMessage(
                "{rt3}" .. name1 .. " " .. rscreleasedtxt1 .. " " .. time .. " " ..
                    rscsec .. " " .. rscreleasedtxt2 .. ", " .. rscreleasedtxt7 ..
                    ": " .. buff .. "!", "WHISPER", nil, name2)
        end
    end
    if nr == 2 then
        if UnitSex(name1) and UnitSex(name1) == 3 then
            SendChatMessage(
                "{rt4}" .. name1 .. " - " .. time .. " " .. rscsec .. ", " ..
                    rscreleasedtxt6f .. ": " .. buff .. "!", "WHISPER", nil,
                name2)
        else
            SendChatMessage(
                "{rt4}" .. name1 .. " - " .. time .. " " .. rscsec .. ", " ..
                    rscreleasedtxt6 .. ": " .. buff .. "!", "WHISPER", nil,
                name2)
        end
    end
end
