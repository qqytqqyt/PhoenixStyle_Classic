...
function psiccvalitriapristnoob()
    ...
    for i, getcrash in ipairs(psiccvalitrifailspirit1) do
        if getcrash == arg4 then
            local pstimelocal =
                math.ceil((10 - (psiccschet2 - psiccschet)) * 10) / 10
            pstimelocal = math.ceil((psiccvalitrifailspirit2[i] + pstimelocal) *
                                        10) / 10
            psiccvalitrifailspirit2[i] = pstimelocal
            bililine = 1
        end
    end
    if (bililine == 0) then
        table.insert(psiccvalitrifailspirit1, arg4)
        local pstimelocal = math.ceil((10 - (psiccschet2 - psiccschet)) * 10) /
                                10
...
function psficcvalytriaafterf()
    if (psicgalochki[11][1] == 1 and psicgalochki[11][2] == 1) then
        ...
        reportafterboitwotab(whererepiccchat[psicchatchoose[11][2]], true,
                             vezaxname, vezaxcrash, 1, 7)
    end

    if (psicgalochki[11][1] == 1 and psicgalochki[11][3] == 1) then
        strochkavezcrash = "{rt7} " .. psiccfailtxt102
        reportafterboitwotab(whererepiccchat[psicchatchoose[11][3]], true,
    ...
    if (psicgalochki[11][1] == 1 and psicgalochki[11][4] == 1 and psiccvalpriest) then
        ...
        reportafterboitwotab(whererepiccchat[psicchatchoose[11][4]], true,
    ...
    if (psicgalochki[11][1] == 1 and psicgalochki[11][4] == 1 and psiccvalpriest) then
        ...
        reportafterboitwotab(whererepiccchat[psicchatchoose[11][4]], true,
                             psiccvalitrifailspirit1, psiccvalitrifailspirit2,
                             1, 7)
    end

    if (psicgalochki[11][1] == 1 and psicgalochki[11][6] == 1) then
        strochkadamageout = "{rt5} " .. psiccbossfail105t
        reportfromtwodamagetables3(whererepiccchat[psicchatchoose[11][6]], true,
                                   7)
    end
    ...
    if psiccsavfile and (psicclastsaverep == nil or (psicclastsaverep and
        ...
        strochkavezcrash = "{rt7} " .. psiccfailtxt101
        reportafterboitwotab(whererepiccchat[psicchatchoose[11][2]], true,
                             vezaxname, vezaxcrash, nil, 15, 0, 1)
        strochkavezcrash = "{rt7} " .. psiccfailtxt102
        reportafterboitwotab(whererepiccchat[psicchatchoose[11][3]], true,
                             vezaxname2, vezaxcrash2, nil, 15, 0, 1)
        strochkavezcrash = "{rt1} " .. psiccfailtxt103
        reportafterboitwotab(whererepiccchat[psicchatchoose[11][4]], true,
                             vezaxname3, vezaxcrash3, nil, 15, 0, 1)
        if psiccvalpriest then
            strochkavezcrash = "{rt8} " .. psiccfailtxt1032
            reportafterboitwotab(whererepiccchat[psicchatchoose[11][4]], true,
                                 psiccvalitrifailspirit1,
                                 psiccvalitrifailspirit2, 1, 7, 0, 1)
            strochkadamageout = "{rt5} " .. psiccbossfail105t
            reportfromtwodamagetables3(whererepiccchat[psicchatchoose[11][6]],
                                       true, 10, 0, 1)
        end
...
function psficcvalytriaresetall()
    ...
    if psdamagename3 then table.wipe(psdamagename3) end
    if psdamagevalue3 then table.wipe(psdamagevalue3) end
