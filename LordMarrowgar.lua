...
function psficclordmrem(idbone, bonetime, raidnick)
    if psbossblock == nil and psicctablebones and #psicctablebones > 0 then
        ...
        while i <= #psicctablebones do
            if string.sub(psicctablebones[i],
                          string.find(psicctablebones[i], "+") + 1) == idbone then
                if psiccbonetime == nil then
                    psiccbonetime = {1000, "0", 0, "0"}
                end
                local bonekilled = bonetime -
                                       tonumber(
                                           string.sub(psicctablebones[i], 1,
                                                      string.find(
                                                          psicctablebones[i],
                                                          "+") - 1))
                ...
                if (psicgalochki[2][1] == 1 and psicgalochki[2][3] == 1) then
                    ...
                    pszapuskanonsa(whererepiccchat[psicchatchoose[2][3]],
                                   "{rt" .. lolmark .. "}" .. psicclordann3 ..
                                       bonekilled .. " " .. pssec .. " (" ..
                                       raidnick .. ").")
                end
...
function psficclorlmafterf()
    if (psicgalochki[2][1] == 1 and psicgalochki[2][2] == 1) then
        if psiccbonetime then
            ...
            pszapuskanonsa(whererepiccchat[psicchatchoose[2][2]],
                           "{rt6}" .. psicclordann1 .. psiccbonetime[1] .. " " ..
                               pssec .. " (" .. psiccbonetime[2] .. "), " ..
                               psicclordann2 .. psiccbonetime[3] .. " " .. pssec ..
                               " (" .. psiccbonetime[4] .. ").", 1)
        end
    end
    ...
    if (psicgalochki[2][1] == 1 and psicgalochki[2][4] == 1) then
        ...
        reportafterboitwotab(whererepiccchat[psicchatchoose[2][4]], true,
                             vezaxname, vezaxcrash, 1, 5)
    end
