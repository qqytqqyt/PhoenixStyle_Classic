function iccadddamagetables(nicktoadd, damagetoadd, minus)
    ...
    for i, getcrash in ipairs(psiccdamagenames[1][#psiccdamagenames[1] - minus]) do
        if getcrash == nicktoadd then
            psiccdamagevalues[1][#psiccdamagevalues[1] - minus][i] =
                psiccdamagevalues[1][#psiccdamagevalues[1] - minus][i] +
                    damagetoadd
            bililine = 1
        end
    ...
    if (bililine == 0) then
        table.insert(psiccdamagenames[1][#psiccdamagenames[1] - minus],
                     nicktoadd)
        table.insert(psiccdamagevalues[1][#psiccdamagevalues[1] - minus],
                     damagetoadd)
    end
...
function iccdamagetablsort(minus)
    ...
    while vzxnnw > 1 do
        if psiccdamagevalues[1][#psiccdamagevalues[1] - minus][vzxnnw] >
            psiccdamagevalues[1][#psiccdamagevalues[1] - minus][vzxnnw - 1] then
            local vezaxtempv1 = psiccdamagevalues[1][#psiccdamagevalues[1] -
                                    minus][vzxnnw - 1]
            local vezaxtempv2 =
                psiccdamagenames[1][#psiccdamagenames[1] - minus][vzxnnw - 1]
            psiccdamagevalues[1][#psiccdamagevalues[1] - minus][vzxnnw - 1] =
                psiccdamagevalues[1][#psiccdamagevalues[1] - minus][vzxnnw]
            psiccdamagenames[1][#psiccdamagenames[1] - minus][vzxnnw - 1] =
                psiccdamagenames[1][#psiccdamagenames[1] - minus][vzxnnw]
            psiccdamagevalues[1][#psiccdamagevalues[1] - minus][vzxnnw] =
                vezaxtempv1
            psiccdamagenames[1][#psiccdamagenames[1] - minus][vzxnnw] =
                vezaxtempv2
    ...
    if #psiccquantooze == 1 then
        ...
        for i = 1, GetNumGroupMembers() do
            ...
            if pssubgroup <= psgrups then
                table.insert(psiccswitchnames[1][#psiccswitchnames[1]],
                             (GetRaidRosterInfo(i)))
                ...
                table.insert(psiccindexclass[1][#psiccindexclass[1]],
                             rsccodeclass)
...
function openicccombarchoose()
    if not DropDownicccombarchoose then
        CreateFrame("Frame", "DropDownicccombarchoose", PSFiccdamageinfo,
    ...
    for i = 1, 3 do if psicccombatinfo[i] == nil then items[i] = nil end end
    ...
    if psiccdmg1 == nil and #psicccombatinfo > 0 then psiccdmg1 = 1 end
...
function openicceventchoose()
    if not DropDownicceventchoose then
        CreateFrame("Frame", "DropDownicceventchoose", PSFiccdamageinfo,
                    "UIDropDownMenuTemplate")
    end
...
function openiccquantname()
    if not DropDowniccquantname then
        CreateFrame("Frame", "DropDowniccquantname", PSFiccdamageinfo,
                    "UIDropDownMenuTemplate")
    end
...
function openicclogchat()
    if not DropDownicclogchat then
        CreateFrame("Frame", "DropDownicclogchat", PSFiccdamageinfo,
                    "UIDropDownMenuTemplate")
    end
...
function psiccdmgshow(ar1, ar2, ar3)
    if psiccdmgtxt1 then
        if ar1 == nil or ar2 == nil then
            ...
            -- определяем тип боя
            if psiccindexboss[ar1] == 1 or psiccindexboss[ar1] == 3 or
                psiccindexboss[ar1] == 4 then

                -- проф & lich king & saurfang
                local psstrochka = "|CFFFFFF00" .. psicceventsnames[ar1][ar2] ..
                                       " " .. psiccdmgfrom .. ": |r"
                ...
                if #psiccdamagenames[ar1][ar2] > 0 then
                    for i = 1, maxnick do
                        ...
                        if (string.len(psiccdamagevalues[ar1][ar2][i])) > 3 then
                            psinfoshieldtempdamage = string.sub(
                                                         psiccdamagevalues[ar1][ar2][i],
                                                         1, string.len(
                                                             psiccdamagevalues[ar1][ar2][i]) -
                                                             3)
                            psinfoshieldtempdamage =
                                psinfoshieldtempdamage .. "k"
                        else
                            psinfoshieldtempdamage =
                                psiccdamagevalues[ar1][ar2][i]
                        end
                        ...
                        for j = 1, #psiccswitchnames[ar1][ar2] do
                            if psiccswitchnames[ar1][ar2][j] ==
                                psiccdamagenames[ar1][ar2][i] then
                                pstimesw = psiccswitchtime[ar1][ar2][j]
                                rsccodeclass = psiccindexclass[ar1][ar2][j]
                            end
                        end
                        ...
                        if psiccindexboss[ar1] == 1 then
                            if pstimesw == "??" and
                                psiccdamagenames[ar1][ar2][i] == psiccprofadd then

                                for j = 1, #psiccswitchnames[ar1][ar2] do
                                    if psiccswitchnames[ar1][ar2][j] ==
                                        psiccprofinabom[ar1][1] then
                                        pstimesw = psiccswitchtime[ar1][ar2][j]
                                    end
                                end
                        ...
                        local tablecolor = {
                            "|CFFC69B6D", "|CFFC41F3B", "|CFFF48CBA",
                            "|CFFFFFFFF", "|CFF1a3caa", "|CFFFF7C0A",
                            "|CFFFFF468", "|CFF68CCEF", "|CFF9382C9",
                            "|CFFAAD372"
                        }

                        if rsccodeclass == nil then
                            rsccodeclass = 0
                        end

                        if rsccodeclass > 0 then
                            colorname = tablecolor[rsccodeclass] ..
                                            psiccdamagenames[ar1][ar2][i] ..
                                            "|r"
                        end

                        if i == maxnick then
                            psstrochka =
                                psstrochka .. colorname .. " (" ..
                                    psinfoshieldtempdamage .. " - " .. pstimesw ..
                                    "s)" .. pstochki
            ...
            if psiccindexboss[ar1] == 2 then
                ...
                for i = 1, #psiccswitchtime[ar1][ar2] do
                    if psiccswitchtime[ar1][ar2][i] == "--" then
                    else
                        maxnick = maxnick + 1
                    end
                end
                ...
                if #pscounciltablesort1 > 0 then
                    for i = 1, maxnick do
                        ...
                        if rsccodeclass > 0 then
                            colorname = tablecolor[rsccodeclass] ..
                                            pscounciltablesort1[i] .. "|r"
                        end

                        if i == maxnick then
                            psstrochka =
                                psstrochka .. colorname .. " (" .. pstimesw ..
                                    "s)" .. pstochki
                            psiccdmgtxt1:SetText(psstrochka)
                            psiccdmgtxt2:SetText(psicccombatinfo[ar1])
                        else
...
function psciccreportdmgshown(chat, ar1, ar2, ar3, boy, quant, zapusk)
    ...
    if ar1 and ar2 then
        ...
        if psiccindexboss[ar1] == 1 or psiccindexboss[ar1] == 3 or
            ...
            -- проф & lich king & saurfang
            local psstrochkab = "{rt4} " .. psicceventsnames[ar1][ar2] .. " " ..
                                    psiccdmgfrom2
            local psstrochka = ""
            local psstrochka2 = ""
            local maxnick = #psiccdamagenames[ar1][ar2]
            ...
            if #psiccdamagenames[ar1][ar2] > 0 then
                for i = 1, maxnick do
                    ...
                    -- abom
                    if psiccindexboss[ar1] == 1 then
                        if pstimesw == "??" and psiccdamagenames[ar1][ar2][i] ==
                            psiccprofadd then
                    ...
                    if i == maxnick then
                        ...
                        if zapusk then
                            ...
                            psfchatsendreports(chat, psstrochkab, psstrochka,
                                               psstrochka2)
                        end
                    else
                        if string.len(psstrochka) > 230 and quant == 2 then
            ...
            if #pscounciltablesort1 > 0 then
                for i = 1, maxnick do
                    ...
                    if i == maxnick then
                        if string.len(psstrochka) > 230 and quant == 2 then
                            psstrochka2 =
                                psstrochka2 .. pscounciltablesort1[i] .. " (" ..
                                    pstimesw .. "s)" .. pstochki
                        else
                            psstrochka =
                                psstrochka .. pscounciltablesort1[i] .. " (" ..
                                    pstimesw .. "s)" .. pstochki
                        end
                        if zapusk then
                            pszapuskanonsa(chat, psstrochka)
